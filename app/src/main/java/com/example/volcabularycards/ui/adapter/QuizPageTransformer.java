...
public class QuizPageTransformer implements ViewPager2.PageTransformer {
    ...
    public QuizPageTransformer(ViewPager2 viewPager) {
    ...
    public void transformPage(@NonNull View page, float position) {
        ...
        View cardView = page.findViewById(R.id.quiz_view);
        ...
        if (recyclerView != null) {
            ...
            if (holder != null && holder.itemView == page) {
                ...
                if (adapterPosition != RecyclerView.NO_POSITION) {
                    ...
                    if (adapter instanceof QuizFragmentAdapter) {
                        QuizBaseFragment fragment = (QuizBaseFragment) 
                            ((QuizFragmentAdapter) adapter).getFragment(adapterPosition);
                        if (fragment != null) {
                            frontCard = fragment.getQuizCardFront();
                            if (backCard == null) {
                                backCard = fragment.getQuizCardBack();
                            }
                        }
                    }
                }
                ...
                if (adapter instanceof QuizFragmentAdapter) {
                    ...
                    if (fragment != null) {
                        if (Math.abs(position) < 0.1f) {
                            //fragment.showFront();
                            //Log.d("QuizPageTransformer", "notifyFragmentOfPositionChange: showFront");
                        } else if (Math.abs(position) > 0.5f) {
                            //fragment.showBack();
                            //Log.d("QuizPageTransformer", "notifyFragmentOfPositionChange: showBack");
                        }
                    }
                }
            }
